...
def calculate_transmission_loss(
    ...
    Supports scalar or array distances. Uses the practical spreading model from DEPONS:
    TL = β * log10(r) + α * r

    For distances <= 0, TL is defined as 0 (source at listener).
    """
    # Handle scalar inputs separately for simple return types
    dist_arr = np.asarray(distance_m)
    ...
    if dist_arr.shape == ():
        ...
        if d <= 0:
            return 0.0
        d = max(1.0, d)
        spreading_loss = beta_hat * np.log10(d)
        absorption_loss = alpha_hat * d
        return spreading_loss + absorption_loss

    # Vectorized path
    dist = np.asarray(dist_arr, dtype=float)
    # Avoid log(0) by using max(1.0, distance) for the log term
    dist_for_log = np.maximum(dist, 1.0)
    spreading_loss = beta_hat * np.log10(dist_for_log)
    absorption_loss = alpha_hat * dist  # absorption uses true distance (can be 0)

    tl = spreading_loss + absorption_loss
    # For any distances <= 0, TL = 0
    tl = np.where(dist <= 0.0, 0.0, tl)
    return tl
...
def response_probability_from_rl(
    ...
    """
    Convert received level (RL) or Signal-to-Noise (SNR) to a response probability using a logistic function.

    p = 1 / (1 + exp(-slope * (value - threshold)))

    Args:
        rl: Received level or SNR (dB) scalar or array
        threshold: Midpoint (dB) where p=0.5
        slope: Steepness per dB

    Returns:
        Probability between 0 and 1 (same shape as rl)
    """
    rl_arr = np.asarray(rl)
    p = 1.0 / (1.0 + np.exp(-slope * (rl_arr - threshold)))
    return float(p) if p.shape == () else p


def combine_rls(*rl_arrays: np.ndarray) -> np.ndarray:
    """
    Combine multiple RL arrays (dB) into a single ambient RL array using linear power summation.
    Accepts arrays of equal length; elements with negative large values (<= -900) are treated as absent.
    """
    if not rl_arrays:
        return np.array([])
    n = len(rl_arrays[0])
    lin = np.zeros(n, dtype=np.float64)
    for arr in rl_arrays:
        if arr is None:
            continue
        arr = np.array(arr)
        present = arr > -900
        lin[present] += 10.0 ** (arr[present] / 10.0)
    rl_combined = np.full(n, -999.0, dtype=np.float32)
    nonzero = lin > 0
    rl_combined[nonzero] = 10.0 * np.log10(lin[nonzero])
    return rl_combined
