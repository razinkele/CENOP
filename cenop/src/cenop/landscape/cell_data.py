...
def create_landscape_from_depons(
    ...
    if os.path.exists(blocks_file):
        ...
        print(f"Loaded blocks: unique values = {np.unique(cell_data._blocks)[:10]}...")
    else:
        print(f"blocks.asc not found, using default zeros")
    ...
    if os.path.exists(disttocoast_file):
        print(f"Loading distance to coast from {disttocoast_file}...")
        disttocoast_array, _ = load_bathymetry_from_asc(disttocoast_file)
        cell_data._dist_to_coast = disttocoast_array
        print(f"Loaded disttocoast: range = {disttocoast_array.min():.1f} to {disttocoast_array.max():.1f}")
    else:
        print(f"disttocoast.asc not found, using default 10000m")
    ...
    if os.path.exists(sediment_file):
        print(f"Loading sediment from {sediment_file}...")
        sediment_array, _ = load_bathymetry_from_asc(sediment_file)
        cell_data._sediment = sediment_array
        print(f"Loaded sediment: unique values = {np.unique(sediment_array)[:10]}...")
    else:
        print(f"sediment.asc not found, using default 1.0")
