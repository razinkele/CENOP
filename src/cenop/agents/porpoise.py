...
class Porpoise(Agent):
    ...
    def step(
        ...
        deterrence: tuple[float, float] | None = None,
    ) -> None:
        """
    ...
    def _standard_move(self, cell_data: CellData | None, params: SimulationParameters) -> None:
        """
    ...
    def _calculate_turning_angle(
        self, cell_data: CellData | None, params: SimulationParameters
    ) -> float:
        """
    ...
    def _check_water_ahead(
        self, cell_data: CellData, distance: float, params: SimulationParameters
    ) -> bool:
    ...
    def _avoid_land(self, cell_data: CellData, params: SimulationParameters) -> None:
        """
        ...
        for angle in [40, 70, 120]:
            ...
            right_ok = (
                cell_data.is_valid_position(right_x, right_y)
                and cell_data.get_depth(right_x, right_y) >= params.min_depth
            )

            # Check left
            left_x, left_y = self.get_point_ahead(pres_mov, -(angle + rand_offset))
            left_ok = (
                cell_data.is_valid_position(left_x, left_y)
                and cell_data.get_depth(left_x, left_y) >= params.min_depth
            )
    ...
    def _dispersal_step(self, cell_data: CellData | None, params: SimulationParameters) -> None:
        """
    ...
    def _update_energetic_status(
        self, cell_data: CellData | None, params: SimulationParameters, current_month: int = 6
    ) -> None:
        """
        ...
        consumed = (
            0.001 * scaling_factor * params.e_use_per_30_min
    ...
    def daily_step(
        self, cell_data: CellData | None, params: SimulationParameters, state: SimulationState
    ) -> None:
        """
    ...
    def _check_mortality(self, params: SimulationParameters, state: SimulationState) -> None:
    ...
    def _update_reproduction(self, params: SimulationParameters, state: SimulationState) -> None:
        ...
        if (
            self.age >= self.age_of_maturity
            and self.pregnancy_status == PregnancyStatus.UNABLE_YOUNG
        ):
        ...
        if (
            self.pregnancy_status == PregnancyStatus.READY_TO_MATE
            and day_of_year == self.mating_day
        ):
    ...
    def deter(
        self, strength: float, source_x: float, source_y: float, params: SimulationParameters
    ) -> None:
        """
