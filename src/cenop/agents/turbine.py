...
class Turbine(Agent):
    ...
    def should_deter(
        ...
        transmission_loss = params.beta_hat * np.log10(distance_m) + params.alpha_hat * distance_m
    ...
    def get_deterrence_vector(
        self, porpoise_x: float, porpoise_y: float, strength: float, deter_coeff: float = 0.07
    ) -> tuple[float, float]:
        """Calculate deterrence vector for a porpoise."""
        return calculate_deterrence_vector(
            porpoise_x, porpoise_y, self.x, self.y, strength, deter_coeff
        )
    ...
    def load_from_file(
        ...
        with open(filepath) as f:
            ...
            for i, line in enumerate(f):
                ...
                if len(cols) < 4:
                    logger.warning(
                        "Turbine file line %d: expected >= 4 columns, got %d — skipping",
                        i + 2,
                        len(cols),
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        "Turbine file line %d: invalid numeric value (%s) — skipping", i + 2, e
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        "Turbine file line %d: invalid tick value (%s) — using defaults", i + 2, e
                    )
                    raw_start = 0
                    raw_end = 2147483647
...
class TurbineManager:
    ...
    def __init__(self, turbines: list[Turbine] | None = None):
        ...
        for turbine in self.get_active_turbines():
            ...
            if should_deter and strength > 0:
                dx, dy = turbine.get_deterrence_vector(
                    porpoise_x, porpoise_y, strength, params.deter_coeff
                )
    ...
    def calculate_aggregate_deterrence_vectorized(
        ...
        for turbine in active_turbines:
            ...
            if params.deter_probabilistic:
                ...
                p = response_probability_from_rl(rl, threshold, params.deter_response_slope)
    ...
    def load_from_file(
        ...
        self.turbines = Turbine.load_from_file(filepath, utm_origin_x, utm_origin_y, cell_size)
