...
class RefMem:
    ...
    def __init__(
        self, max_entries: int = 120, decay_satiation: float = 0.03, decay_reference: float = 0.03
    ):
        """
    ...
    def update(self) -> None:
        ...
        # Remove weak entries
        self._entries = [e for e in self._entries if e.strength > 0.001]

    def get_best_memory(self) -> tuple[float, float] | None:
        """
    ...
    def get_nearest_memory(
        self, x: float, y: float, min_strength: float = 0.1
    ...
    def get_memories_in_direction(
        self, x: float, y: float, heading: float, cone_angle: float = 90.0
