    ...
    def __post_init__(self):
        if self.initial_population > 0:
            self.population_change_pct = (
                (self.final_population - self.initial_population) / self.initial_population * 100
            )
        else:
...
class BatchRunner:
    ...
    def run(self, progress: bool = True) -> list[BatchResult]:
        ...
        if self.config.parallel and total_runs > 1:
            self.results = self._run_parallel(combinations, progress)
        else:
    ...
    def _run_sequential(
        self, combinations: list[dict[str, Any]], progress: bool = True
    ) -> list[BatchResult]:
        ...
        for combo in combinations:
            for rep in range(self.config.replicates):
                ...
                if progress:
                    ...
                    param_str = ", ".join(
                        f"{k}={v}" for k, v in combo.items() if k in self.config.variations
                    )
                    logger.info(
                        "  [%d/%d] (%.0f%%) %s, rep=%d", run_id + 1, total, pct, param_str, rep + 1
                    )
    ...
    def _run_parallel(
        ...
        try:
            with ProcessPoolExecutor(max_workers=self.config.max_workers) as executor:
                futures = {executor.submit(self._run_single, *task): task[0] for task in tasks}
            ...
            logger.warning(
                "Parallel execution failed: %s, falling back to sequential", e, exc_info=True
            )
    ...
    def _run_single(self, run_id: int, params: dict[str, Any], seed: int) -> BatchResult:
    ...
    def export_results(
        self, results: list[BatchResult] | None = None, filename: str = "batch_results.csv"
    ) -> Path:
        """
    ...
    def export_summary(
        self, results: list[BatchResult] | None = None, filename: str = "batch_summary.json"
    ) -> Path:
    ...
    def get_results_by_parameter(self, param_name: str) -> dict[Any, list[BatchResult]]:
    ...
    def calculate_sensitivity(
        self, param_name: str, metric: str = "final_population"
    ) -> dict[str, Any]:
        """
        ...
        sensitivity = {"parameter": param_name, "metric": metric, "values": {}}
...
def run_scenario_comparison(
    scenarios: dict[str, dict[str, Any]], replicates: int = 5, output_dir: str = "output/scenarios"
