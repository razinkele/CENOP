...
class OutputWriter:
    ...
    def __init__(self, config: OutputConfig | None = None):
        ...
        # File handles
        self._population_file: TextIO | None = None
        self._statistics_file: TextIO | None = None
        self._dispersal_file: TextIO | None = None
        self._mortality_file: TextIO | None = None
        self._energy_file: TextIO | None = None

        # CSV writers
        self._population_writer: csv.DictWriter | None = None
        self._statistics_writer: csv.DictWriter | None = None
        self._dispersal_writer: csv.DictWriter | None = None
        self._mortality_writer: csv.DictWriter | None = None
        self._energy_writer: csv.DictWriter | None = None
        ...
        self._prev_active_mask: np.ndarray | None = None
        self._prev_dispersing: np.ndarray | None = None
    ...
    def _open_population_file(self) -> None:
        ...
        self._population_file = open(filepath, "w", newline="")
        self._population_writer = csv.DictWriter(
            self._population_file,
            fieldnames=[
                "tick",
                "day",
        ...
        self._population_writer.writeheader()
    ...
    def _open_statistics_file(self) -> None:
        ...
        self._statistics_file = open(filepath, "w", newline="")
        self._statistics_writer = csv.DictWriter(
            self._statistics_file,
            fieldnames=[
                "tick",
                "day",
        ...
        self._statistics_writer.writeheader()
    ...
    def _open_dispersal_file(self) -> None:
        ...
        self._dispersal_file = open(filepath, "w", newline="")
        self._dispersal_writer = csv.DictWriter(
            self._dispersal_file,
            fieldnames=[
                "tick",
                "day",
        ...
        self._dispersal_writer.writeheader()
    ...
    def _open_mortality_file(self) -> None:
        ...
        self._mortality_file = open(filepath, "w", newline="")
        self._mortality_writer = csv.DictWriter(
            self._mortality_file,
            fieldnames=["tick", "day", "porpoise_id", "age", "energy", "cause", "x", "y"],
            delimiter="\t",
        )
        self._mortality_writer.writeheader()
    ...
    def _open_energy_file(self) -> None:
        ...
        self._energy_file = open(filepath, "w", newline="")
    ...
    def _detect_dispersal(self, sim: Simulation) -> None:
        ...
        if self._prev_dispersing is not None:
            ...
            for idx in dispersing_indices:
                ...
                target_x = (
                    pop.dispersal_target_x[idx]
                    if hasattr(pop, "dispersal_target_x")
                    else pop.x[idx]
                )
                target_y = (
                    pop.dispersal_target_y[idx]
                    if hasattr(pop, "dispersal_target_y")
                    else pop.y[idx]
                )
                target_dist = (
                    pop.dispersal_target_distance[idx]
                    if hasattr(pop, "dispersal_target_distance")
                    else 0
                )
    ...
    def _write_statistics(self, sim: Simulation) -> None:
        ...
        for idx in active_indices:
            row = {
                ...
                "is_dispersing": (
                    int(pop.is_dispersing[idx]) if hasattr(pop, "is_dispersing") else 0
                ),
...
def create_simulation_output(
    sim: Simulation, output_dir: str = "output", run_id: str | None = None
) -> OutputWriter:
    """
    ...
    """
    config = OutputConfig(output_dir=output_dir, run_id=run_id)

    return OutputWriter(config)


def run_with_output(
    sim: Simulation, output_dir: str = "output", progress: bool = True
) -> OutputWriter:
    """
