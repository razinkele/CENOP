...
class Scheduler:
    ...
    def schedule_repeating(
        ...
        """
        task = ScheduledTask(
            action=action, start_tick=start, interval=interval, priority=priority, name=name
        )
    ...
    def schedule_once(
        ...
        """
        task = ScheduledTask(
            action=action, start_tick=tick, interval=0, priority=priority, name=name
        )
        self._tasks.append(task)
        self._sort_tasks()
        return task
    ...
    def execute_tick(self, tick: int) -> None:
        ...
        self._tasks = [t for t in self._tasks if t.interval > 0 or t.start_tick > tick]
