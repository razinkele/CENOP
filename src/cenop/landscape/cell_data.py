...
class CellData:
    ...
    def __init__(self, landscape_name: str, data_dir: str = None):
        ...
        if data_dir is None:
            ...
            package_root = Path(
                __file__
            ).parent.parent.parent.parent  # src/cenop/landscape -> cenop
            self.data_dir = package_root / "data"
        else:
            self.data_dir = Path(data_dir)

        self.metadata: LandscapeMetadata | None = None

        # Data arrays
        self._depth: np.ndarray | None = None
        self._dist_to_coast: np.ndarray | None = None
        self._sediment: np.ndarray | None = None
        self._food_prob: np.ndarray | None = None
        self._food_value: np.ndarray | None = None
        self._blocks: np.ndarray | None = None
        self._entropy: np.ndarray | None = None  # Shape: (12, height, width)
        self._salinity: np.ndarray | None = None  # Shape: (12, height, width)

        self._demand_grid: np.ndarray | None = None
    ...
    def load(self) -> None:
        ...
        if self._entropy is not None:
            ...
            self._food_value = np.where(
                (self._food_prob > 0) & (max_ent > 0), max_u * max_ent / mean_max_ent, 0.0
            ).astype(np.float32)
        else:
    ...
    def replenish_food(
        ...
        delta[active] = rate * food[active] * (1.0 - food[active] / k_vals[active])
    ...
    def get_salinity(self, x: float, y: float, month: int | None = None) -> float:
    ...
    def get_max_ent(self, x: float, y: float, month: int | None = None) -> float:
    ...
    def get_current_max_ent(self) -> np.ndarray | None:
...
def load_bathymetry_from_asc(filepath: str) -> tuple[np.ndarray, LandscapeMetadata]:
    ...
    with open(filepath) as f:
        ...
        for expected in expected_headers:
            ...
            if not line:
                raise ValueError(
                    f"ASC header incomplete: missing '{expected}' " f"(file: {filepath})"
                )
            parts = line.split()
            if len(parts) < 2:
                raise ValueError(
        ...
        try:
            ...
            raise ValueError(f"ASC header has non-numeric value: {e} (file: {filepath})") from e
        ...
        for line_num, line in enumerate(f, start=7):
            try:
                ...
                raise ValueError(
                    f"ASC data has non-numeric value at line {line_num}: {e} " f"(file: {filepath})"
                ) from e
            data.extend(values)
...
def create_landscape_from_depons(depons_data_dir: str = None, food_prob: float = 0.5) -> CellData:
    """
    ...
    logger.info(
        "Loaded bathymetry: %dx%d, depth range: %.1f to %.1fm",
        metadata.nrows,
        metadata.ncols,
        depth_array.min(),
        depth_array.max(),
...
def create_homogeneous_landscape(
    width: int = 400, height: int = 400, depth: float = 30.0, food_prob: float = 0.5
) -> CellData:
    """
    ...
    cell_data.metadata = LandscapeMetadata(
        ncols=width, nrows=height, xllcorner=0.0, yllcorner=0.0, cellsize=400.0
    )
