...
try:
    ...
    from .simulation_controller import SimulationRunner, create_simulation_from_inputs
...
def _get_transformer(source_crs: str):
    ...
    if source_crs not in _transformers:
        ...
        _transformers[source_crs] = Transformer.from_crs(source_crs, "EPSG:4326", always_xy=True)
    return _transformers[source_crs]


def grid_to_lonlat(grid_x, grid_y, metadata, source_crs: str):
    ...
    try:
        while not runner.is_complete and not stop_event.is_set():
            ...
            if loop_count <= 5 or loop_count % 500 == 0:
                logger.debug(
                    "Loop #%d: tick=%s, pop=%s, year=%s, speed=%.2f, ticks_per_update=%s",
                    loop_count,
                    runner.tick,
                ...
                try:
                    ...
                    if raw_pos.size > 0:
                        ...
                        if meta is not None:
                            ...
                            crs = LANDSCAPE_CRS.get(sim.params.landscape, "EPSG:3035")
                            lons, lats = grid_to_lonlat(raw_pos[:, 1], raw_pos[:, 2], meta, crs)
                ...
                if traces_on and porpoise_positions:
                    ...
                    for row in porpoise_positions:
                        ...
                        if pid not in trail_history:
                            ...
                            trail_history[pid] = deque(trail_history[pid], maxlen=max_points)
                    ...
                    for pid, trail in list(trail_history.items())[:1000]:
                        if len(trail) >= 2:
                            trail_data.append(
                                {
                                    "pid": pid,
                                    "path": [[t[0], t[1], t[2]] for t in trail],
                                }
                            )
                elif not traces_on:
                    trail_history.clear()
        ...
        if runner.is_complete:
            logger.debug(
                "Simulation COMPLETE after %d iterations, years=%s",
                loop_count,
                runner.sim.state.year,
            )
...
def _build_details_modal_content(name, info, warnings):
    ...
    core_list = [
        shiny_ui.tags.li(f"{'✅' if info.get(f, False) else '❌'} {f}") for f in core_files
    ]
    prey = info.get("prey_months", [])
    sal = info.get("salinity_months", [])
...
def server(input, output, session):
    ...
    async def _push_all_layers():
        ...
        layers = [
            ...
            _layer_cache.get(
                "depth-tooltip", scatterplot_layer("depth-tooltip", [], visible=False)
            ),
            _layer_cache.get(
                "foraging-bitmap", bitmap_layer("foraging-bitmap", "", [], visible=False)
            ),
            _layer_cache.get(
                "foraging-tooltip", scatterplot_layer("foraging-tooltip", [], visible=False)
            ),
    ...
    state.selected_preview_file.set({"landscape": "CentralBaltic", "file": "bathy.asc"})
    ...
    def trigger_load_landscape():
        ...
        try:
            ...
            ui.notification_show(
                "Error loading landscape. Check server logs for details.", type="error"
            )
    ...
    def landscape_selector():
        ...
        selected = (
            ...
            else (choices[0] if choices else "Homogeneous")
        )
    ...
    def trigger_load_turbines():
        ...
        try:
            ...
            if turbine_scenario != "off":
                ui.notification_show(
                    f"Loading turbines: {turbine_scenario}...", type="info", duration=3
                )
        except Exception as e:
            logger.error(f"Error loading turbines: {e}", exc_info=True)
            ui.notification_show(
                "Error loading turbines. Check server logs for details.", type="error"
            )
    ...
    def data_available_table():
        ...
        try:
            ...
            ui.notification_show(
                "Error loading landscapes. Check server logs for details.",
                type="error",
                duration=10,
            )
            landscapes = []
            ...
            shiny_ui.tags.tr(
                ...
                shiny_ui.tags.th(
                    shiny_ui.span(
                        "Core files ",
                        shiny_ui.tags.small(
                            "(B=Bathy, D=DistCoast, S=Sediment, Pat=Patches, Blk=Blocks)",
        ...
        if not row_data:
            ...
            for data in row_data:
                ...
                core_files = [
                    ...
                    "Pat",
                    "Blk",
                ]  # Bathy, DistToCoast, Sediment, Patches, Blocks
                ...
                core_cell = shiny_ui.tags.td(
                    " ".join(core_icons_with_labels), style="font-size: 0.85em;"
                )
                prey_cell = shiny_ui.tags.td(
                    str(data["prey_months"]) if data["prey_months"] else "—"
                )
                sal_cell = shiny_ui.tags.td(
                    str(data["salinity_months"]) if data["salinity_months"] else "—"
                )
                ...
                btn = shiny_ui.tags.button(
                    ...
                    **{
                        "data-landscape": name,
                        "onclick": "event.stopPropagation(); Shiny.setInputValue('detail_landscape', this.dataset.landscape, {priority: 'event'});",
                    },
                )
        ...
        table = shiny_ui.tags.table(
            thead, shiny_ui.tags.tbody(*rows), class_="table table-sm table-striped"
        )
        ...
        try:
            ...
            if selected_landscape:
                ...
                state.selected_preview_file.set(
                    {"landscape": selected_landscape, "file": "bathy.asc"}
                )
                logger.info(
                    f"[PREVIEW DEBUG] Preview landscape changed to: {selected_landscape}, file reset to bathy.asc"
                )
        except Exception as e:
            logger.error(
                f"[PREVIEW DEBUG] Error handling preview landscape change: {e}", exc_info=True
            )
    ...
    def data_preview_controls():
        ...
        try:
            selected_landscape = input.preview_landscape()
            logger.info(
                f"[PREVIEW DEBUG] data_preview_controls rendering for landscape: {selected_landscape}"
            )

            if not selected_landscape:
                logger.info("[PREVIEW DEBUG] No landscape selected, returning empty div")
                return ui.div()
            ...
            for f in files:
                ...
                monthly_match = any(
                    pattern in fname_lower for pattern in ["prey", "sal", "temp", "sed"]
                )
            ...
            logger.info(
                f"[PREVIEW DEBUG] Returning file selector with {len(choices)} choices, current: {current_file}"
            )

            return ui.input_select(
                "file_selector", "File:", choices=choices, selected=current_file, width="300px"
            )
        except Exception as e:
            logger.error(f"[PREVIEW DEBUG] Exception in data_preview_controls: {e}", exc_info=True)
            return ui.div(ui.p(f"Error loading file list: {str(e)}", class_="text-danger"))
    ...
    def _handle_file_selector_change():
        ...
        try:
            ...
            if landscape and selected_file:
                logger.info(
                    f"[PREVIEW DEBUG] Setting selected_preview_file to {landscape}/{selected_file}"
                )
                state.selected_preview_file.set({"landscape": landscape, "file": selected_file})
                logger.info(f"[PREVIEW DEBUG] File selector changed to: {selected_file}")
            else:
                logger.warning(
                    f"[PREVIEW DEBUG] Skipping update - landscape: {landscape}, file: {selected_file}"
                )
        except Exception as e:
    ...
    def preview_data_source():
        ...
        try:
            ...
            if total_cells > 100000:  # Very large (e.g., 400x400 = 160,000)
                ...
                max_points = 10000
                sample_step = (
                    1 if valid_cells < max_points else int(np.sqrt(valid_cells / max_points))
                )

            logger.info(
                f"Sampling {filename}: {nrows}x{ncols} grid, step={sample_step}, target ~{max_points} points"
            )
            ...
            for row in range(0, nrows, sample_step):
                for col in range(0, ncols, sample_step):
                    ...
                    if value != nodata_value:
                        ...
                        points.append({"position": [lon, lat], "value": value})
            ...
            logger.error(
                f"[PREVIEW DEBUG] Error loading preview data {landscape}/{filename}: {e}",
                exc_info=True,
            )
            return None
    ...
    def start_simulation():
        ...
        try:
            ...
            from .simulation_controller import (
                create_simulation_from_inputs as create_sim,
            )
            ...
            sim.initialize()
            logger.info(
                "Simulation initialized, pop_size=%s, max_ticks=%s",
                sim.population_size,
                sim.max_ticks,
            )
            ...
            ui.notification_show(
                "Simulation failed. Check server logs for details.", type="error", duration=10
            )
            return
    ...
    def _sync_skip_viz():
        ...
        enabled = input.skip_viz() if hasattr(input, "skip_viz") else False
        with skip_viz_lock:
    ...
    def _sync_trace_settings():
        ...
        enabled = input.show_traces() if hasattr(input, "show_traces") else False
        days = input.trace_length_days() if hasattr(input, "trace_length_days") else 2
        with trace_lock:
    ...
    def poll_simulation():
        ...
        while True:
            ...
            if msg["type"] == "complete":
                ...
                logger.debug(
                    "Poll: Simulation COMPLETE, final history len=%d",
                    len(state.population_history()),
                )
                return
            ...
            if msg["type"] == "update":
                ...
                if msg["should_update_map"]:
                    ...
                    if msg.get("porpoise_trails") is not None:
                        try:
                            state.porpoise_trails.set(msg.get("porpoise_trails"))
                    ...
                    if msg.get("population_snapshot") is not None:
                        try:
                            state.population_snapshot.set(msg.get("population_snapshot"))
                        except (AttributeError, TypeError) as e:
                            logger.warning("Could not update population_snapshot: %s", e)
    ...
    async def _update_depth_layer():
        ...
        try:
            ...
            if depth is None:
                _layer_cache["depth-bitmap"] = bitmap_layer("depth-bitmap", "", [], visible=False)
                _layer_cache["depth-tooltip"] = scatterplot_layer(
                    "depth-tooltip", [], visible=False
                )
                _loaded_data_layers.discard("depth-bitmap")
                return
    ...
    async def _update_foraging_layer():
        ...
        if not loaded_name:
            ...
            _layer_cache["foraging-tooltip"] = scatterplot_layer(
                "foraging-tooltip", [], visible=False
            )
        ...
        try:
            ...
            if food is None:
                _layer_cache["foraging-bitmap"] = bitmap_layer(
                    "foraging-bitmap", "", [], visible=False
                )
                _layer_cache["foraging-tooltip"] = scatterplot_layer(
                    "foraging-tooltip", [], visible=False
                )
    ...
    async def _update_porpoise_layer():
        ...
        if not positions_raw:
            ...
            _layer_cache["porpoise-trails"] = build_porpoise_trails_layer([])
            ...
            if show_traces and trails_raw:
                ...
                for trail in trails_raw[:1000]:
                    ...
                    colored_trails.append(
                        {
                            "path": trail["path"],
                            "color": pid_to_color.get(pid, [0, 150, 255, 240]),
                        }
                    )
                _layer_cache["porpoise-trails"] = build_porpoise_trails_layer(
                    colored_trails, current_time
                )
    ...
    def deterrence_plot():
        ...
        try:
            ...
            if not history:
                return no_data_placeholder(
                    "Deterrence data will appear when simulation runs with turbines/ships."
                )

            deterred_data = [
                {"day": h["day"], "deterred": h.get("deterred_count", 0)} for h in history
            ]
            df = pd.DataFrame(deterred_data)

            if df["deterred"].sum() == 0:
                return no_data_placeholder(
                    "No deterrence events detected. Enable turbines or ships."
                )
    ...
    def noise_map():
        ...
        try:
            ...
            return ui.HTML(
                model_html
                + '<p style="color:#888; font-size:0.8rem;">No noise exposure events recorded yet.</p>'
            )
        except (ValueError, TypeError, IndexError, KeyError) as e:
