...
class TestTurbineDeterrenceIntegration:
    ...
    def test_turbine_manager_creates_deterrence_vectors(self):
        ...
        assert deterred_count >= 2, f"Multiple porpoises should be deterred, got {deterred_count}"
    ...
    def test_turbine_noise_calculation(self):
        ...
        turbine = Turbine(
            ...
            heading=0.0,
            name="Test",
            impact=220.0,  # Realistic pile driving level
            start_tick=0,
            end_tick=1000000,
        )
        turbine._is_active = True
        ...
        for d in distances_cells:
            ...
            print(
                f"Distance: {d} cells ({dist_m:.0f}m), RL: {rl:.1f} dB, "
            ...
            if d <= 2:  # Within ~800m should deter with SL=220
                assert rl > params.deter_threshold, f"RL should exceed threshold at {d} cells"
...
class TestShipDeterrenceIntegration:
    ...
    def test_ship_manager_creates_deterrence_vectors(self):
        ...
        # Create a route
        route = Route(
            name="TestRoute", buoys=[Buoy(x=10, y=50, speed=10), Buoy(x=100, y=50, speed=10)]
        )
    ...
    def test_ship_deterrence_applied_in_simulation(self):
        ...
        params = SimulationParameters(
            porpoise_count=50, sim_years=1, landscape="Homogeneous", ships_enabled=True
        )
        sim = Simulation(params=params)
        sim.initialize()
...
class TestCombinedDeterrence:
    ...
    def test_combined_deterrence_vectors_accumulate(self):
        ...
        print(
            f"Combined deterrence - Max: {np.max(deter):.4f}, "
...
class TestLandscapeDataLoading:
    ...
    def test_homogeneous_landscape_valid(self):
        ...
        landscape = create_homogeneous_landscape(width=250, height=500, depth=25.0, food_prob=0.3)
    ...
    def test_wind_farm_data_loading(self):
        ...
        turbines = Turbine.load_from_file(
            str(dantysk_file), utm_origin_x=0.0, utm_origin_y=0.0, cell_size=400.0
        )
        ...
        t = turbines[0]
        print(
            f"First turbine: id={t.id}, name={t.name}, impact={t.impact}, "
...
class TestDeterrenceStrengthLogging:
    ...
    def test_deter_strength_array_updated(self):
        ...
        params = SimulationParameters(
            porpoise_count=100, sim_years=1, landscape="Homogeneous", turbines="construction"
        )
        sim = Simulation(params=params)
        sim.initialize()
...
class TestEnhancedEnergetics:
    ...
    def test_seasonal_energy_scaling(self):
        ...
        for month in [5, 6, 7, 8, 9]:
            scaling = pop._get_energy_scaling(month, pop.active_mask)
            assert np.allclose(
                scaling, params.e_warm
            ), f"Warm month {month} should have scaling={params.e_warm}"
    ...
    def test_lactation_energy_scaling(self):
        ...
        assert np.allclose(
            scaling[:5], expected_lact
        ), "Lactating females should have higher scaling"

        # Non-lactating: e_warm = 1.3
        assert np.allclose(
            scaling[5:], params.e_warm
        ), "Non-lactating should have base warm scaling"
    ...
    def test_starvation_mortality(self):
        ...
        assert (
            0.49 < yearly_surv_0 < 0.51
        ), f"At energy=0, survival should be ~50%, got {yearly_surv_0}"
        ...
        print(
            f"Survival probabilities: E=0: {yearly_surv_0:.3f}, E=10: {yearly_surv_10:.3f}, E=20: {yearly_surv_20:.3f}"
        )
