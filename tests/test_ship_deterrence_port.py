...
_KATTEGAT_SHIPS_JSON = str(
    Path(__file__).resolve().parent.parent / "data" / "Kattegat" / "ships.json"
)
...
class TestModelExpAndArrays:
    def test_magnitude_uses_exp_link_day(self):
        ...
        linear = (
            m.cship_int_day
            + m.cship_noise_day * ns
            + m.cship_dist_day * ds
            + m.cship_dist_x_noise_day * ns * ds
        )
...
class TestDeterrenceComponents:
    ...
    def test_gate_excludes_rl_at_or_below_tships(self):
        ...
        gdx = np.array([5.0, 5.0])
        gdy = np.array([0.0, 0.0])
    ...
    def test_reacts_iff_u_below_prob(self):
        m = self._model()
        rl = np.array([130.0, 130.0])
        dist_m = np.array([1500.0, 1500.0])
        gdx = np.array([3.0, 3.0])
        gdy = np.array([0.0, 0.0])
...
class TestVectorizedPath:
    ...
    def test_10km_cap_boundary(self):
        ...
        p = self._params()
        near_x = np.array([50.0 + 9900.0 / 400.0])
        y = np.array([50.0])
    ...
    def test_deter_max_distance_tightens_cap(self):
        mgr, s = self._mgr_with_ship()
        p = self._params()
        p.deter_max_distance = 5.0  # km -> cap = 5 km
        x = np.array([50.0 + 6000.0 / 400.0])
        y = np.array([50.0])  # 6 km
    ...
    def test_min_distance_floor(self):
        ...
        x99 = np.array([50.0 + 99.0 / 400.0])
        y = np.array([50.0])  # 99 m
    ...
    def test_deter_coeff_does_not_affect_ship_vector(self):
        ...
        x = np.array([50.0 + 2000.0 / 400.0])
        y = np.array([50.0])  # 2 km
        p1 = self._params()
        p1.deter_coeff = 0.012
        p2 = self._params()
        p2.deter_coeff = 0.5
    ...
    def test_loudest_ship_wins_not_sum(self):
        ...
        p = self._params()
        loud = Ship(id=1, x=48.0, y=50.0, vessel_type=VesselClass.CARGO)
        loud._is_active = True
        loud.noise.base_source_level = 195.0
        quiet = Ship(id=2, x=60.0, y=50.0, vessel_type=VesselClass.CARGO)
        quiet._is_active = True
        quiet.noise.base_source_level = 175.0
        px = np.array([50.0])
        py = np.array([50.0])
        mgr = ShipManager([loud, quiet])
        mgr.enabled = True
        mgr_loud = ShipManager([loud])
        mgr_loud.enabled = True
        mgr_quiet = ShipManager([quiet])
        mgr_quiet.enabled = True
    ...
    def test_order_and_membership_invariance(self):
        ...
        p = self._params()
        a = Ship(id=1, x=49.0, y=50.0, vessel_type=VesselClass.CARGO)
        a._is_active = True
        a.noise.base_source_level = 195.0
        b = Ship(id=2, x=51.0, y=50.0, vessel_type=VesselClass.CARGO)
        b._is_active = True
        b.noise.base_source_level = 190.0
        far = Ship(id=3, x=500.0, y=900.0, vessel_type=VesselClass.CARGO)
        far._is_active = True
        far.noise.base_source_level = 195.0
        px = np.array([50.0])
        py = np.array([50.0])
        ab = ShipManager([a, b])
        ab.enabled = True
...
class TestSimulationDeterminism:
    def _sim_with_ship(self):
        ...
        params = SimulationParameters(
            porpoise_count=50, sim_years=1, random_seed=42, ships_enabled=True
        )
        ...
        loud._is_active = True
        loud.noise.base_source_level = 195.0
        sim._ship_manager.ships = [loud]
        sim._ship_manager.enabled = True
        return sim
...
class TestScalarOracleConsistency:
    def test_scalar_matches_kernel(self):
        ...
        s._is_active = True
        s.noise.base_source_level = 180.0
        ...
        assert (
            should is True
        )  # u=0 + 180 dB ship at 2 km must react; guards against both-False pass
...
class TestEdgeCases:
    ...
    def test_nodata_cell_gives_zero_rl_weston(self):
        ...
        params = SimulationParameters()
        params.weston_flux_percell = True
        land = create_homogeneous_landscape(width=100, height=100, depth=20.0, food_prob=0.5)
        land._depth[:] = -9999.0  # all NODATA depth
        s = Ship(id=1, x=50.0, y=50.0, vessel_type=VesselClass.CARGO)
        s._is_active = True
        s.noise.base_source_level = 210.0
        mgr = ShipManager([s])
        mgr.enabled = True
        px = np.array([50.5])
        py = np.array([50.0])
...
class TestIntegration:
    def _sim(self, source_level):
        ...
        params = SimulationParameters(
            porpoise_count=200, sim_years=1, random_seed=42, ships_enabled=True
        )
        land = create_homogeneous_landscape(width=100, height=100, depth=20.0, food_prob=0.5)
        sim = Simulation(params=params, cell_data=land, seed=42)
        sim.initialize()
        ...
        ship.noise.base_source_level = source_level
        sim._ship_manager.ships = [ship]
        sim._ship_manager.enabled = True
        return sim
...
class TestCharacterizationSnapshot:
    ...
    def test_kernel_snapshot_day(self):
        ...
        gdx = np.array([2.0, -7.5, 1.25])
        gdy = np.array([0.0, 0.0, 0.0])
...
class TestVectorizedPerfRefactor:
    def test_mixed_in_and_out_of_range_correct(self):
        ...
        s = Ship(id=1, x=50.0, y=50.0, vessel_type=VesselClass.CARGO)
        s._is_active = True
        s.noise.base_source_level = 205.0
        mgr = ShipManager([s])
        mgr.enabled = True
    ...
    def test_seed_order_invariance_still_holds(self):
        ...
        def mk(sid, sx):
            ...
            s._is_active = True
            s.noise.base_source_level = 195.0
            return s
...
class TestJomopansSourceLevel:
    ...
    def test_explicit_override_wins(self):
        ...
        n = ShipNoise(
            vessel_class=VesselClass.CARGO, length=200.0, speed=12.0, base_source_level=170.0
        )
...
class TestShipJsonLoader:
    def test_type_string_mapping_all_kattegat_types(self):
        ...
        from cenop.agents.ship import VesselClass, _vessel_class_from_type

        cases = {
            "Bulker": VesselClass.BULKER,
            "Containership": VesselClass.CONTAINER,
            "Tanker": VesselClass.TANKER,
            "Government/Research": VesselClass.GOVERNMENT,
            "Cruise": VesselClass.CRUISE,
            "Dredger": VesselClass.DREDGER,
            "Passenger": VesselClass.PASSENGER,
            "Tug": VesselClass.TUG,
            "Recreational": VesselClass.RECREATIONAL,
            "Fishing": VesselClass.FISHING,
            "Naval": VesselClass.NAVAL,
            "Other": VesselClass.OTHER,
        }
    ...
    def test_loader_reads_type_length_and_no_forced_impact(self):
        ...
        mgr = ShipManager()
        mgr.load_from_json(
            _KATTEGAT_SHIPS_JSON, utm_origin_x=529473.0, utm_origin_y=5972242.0, cell_size=400.0
        )
        assert mgr.count > 0
...
class TestTurbineOnlyDispersal:
    def _pop(self):
        ...
        pop.dispersal_start_x[0] = pop.x[0]
        pop.dispersal_start_y[0] = pop.y[0]
...
class TestTurbineOnlyDispersalJax:
    def test_jax_dispersal_uses_turbine_strength(self):
        ...
        zeros = jnp.zeros(n)
        ddt = jnp.zeros(n)
        dde = jnp.zeros(n, dtype=jnp.int32)
        x = jnp.zeros(n)
        y = jnp.zeros(n)
        eh = jnp.zeros((n, 8))
...
class TestSharedReceivedLevel:
    def test_non_weston_uses_alpha_beta(self):
        ...
        px = np.array([50.0])
        py = np.array([50.0])
        rl = _ship_received_level(180.0, dist_m, px, py, p, cell_data=None, month=1, weston=False)
    ...
    def test_weston_nodata_gives_zero(self):
        ...
        px = np.array([50.0])
        py = np.array([50.0])
        rl = _ship_received_level(210.0, dist_m, px, py, p, cell_data=land, month=1, weston=True)
        assert rl[0] == 0.0
...
class TestScalarAggregatorTL:
    ...
    def test_scalar_uses_weston_when_enabled(self):
        ...
        gdx = np.array([px - 50.0])
        gdy = np.array([0.0])
        rl_w = _ship_received_level(
            s.noise.get_source_level(), dist_m, np.array([px]), np.array([py]), p, land, 1, True
        )
        rl_ab = _ship_received_level(
            s.noise.get_source_level(), dist_m, np.array([px]), np.array([py]), p, None, 1, False
        )
        ...
        vx_w = float(
            m.deterrence_components(
                rl_w, dist_m, gdx, gdy, True, np.array([0.0]), p.deter_ships_min_db
            )[0][0]
        )
        vx_ab = float(
            m.deterrence_components(
                rl_ab, dist_m, gdx, gdy, True, np.array([0.0]), p.deter_ships_min_db
            )[0][0]
        )
    ...
    def test_scalar_without_celldata_uses_alpha_beta(self):
        ...
        vx_ref = float(
            ShipDeterrenceModel().deterrence_components(
                rl,
                np.array([2000.0]),
                np.array([px - 50.0]),
                np.array([0.0]),
...
class TestShipPrevPosition:
    ...
    def test_update_records_pre_move_position(self):
        from cenop.agents.ship import Buoy, Route, Ship, VesselClass

        route = Route(buoys=[Buoy(x=0.0, y=0.0, speed=10.0), Buoy(x=100.0, y=0.0, speed=10.0)])
    ...
    def test_update_inactive_leaves_prev_equal_current(self):
        ...
        s.tick_start = 10
        s.tick_end = 20  # inactive at tick 1
        s.update(1)
...
class TestSubTickInterpolation:
    ...
    def _kernel_vec(self, s, px, py, p, sub_x, sub_y, cell=400.0):
        ...
        gdx = np.array([px - sub_x])
        gdy = np.array([py - sub_y])
    ...
    def test_substep_endpoints_exclude_start_include_end(self):
        ...
        mgr.enabled = True
        px = np.array([10.0])
        py = np.array([5.0])  # near the END (10,0), north
        dx, dy = mgr.calculate_aggregate_deterrence_vectorized(px, py, p, _force_u=0.0)
        exp_x = exp_y = 0.0
        for i in range(1, 31):
    ...
    def test_per_slot_max_rl_ship_wins(self):
        ...
        def slot_rl_vec(s, i):
            ...
            gdx = np.array([px[0] - sub_x])
            gdy = np.array([py[0] - sub_y])
            dist_m = np.array([max(float(np.hypot(gdx[0] * cell, gdy[0] * cell)), 1.0)])
            rl = max(
                0.0,
                float(
    ...
    def test_gated_nonreacting_winner_contributes_zero(self):
        ...
        px = np.array([50.0])
        py = np.array([50.0])
        mgr = ShipManager([loud])
        mgr.enabled = True
        dx, dy = mgr.calculate_aggregate_deterrence_vectorized(
            px, py, p, _force_u=1.0
        )  # never react
    ...
    def test_louder_ship_fully_blocks_quieter(self):
        ...
        def loud():
            return self._ship(1, 49.0, 50.0, sl=205.0)  # closer + louder -> higher RL

        def quiet():
            return self._ship(2, 45.0, 50.0, sl=185.0)  # farther + quieter, still gated

        px = np.array([50.0])
        py = np.array([50.0])
        both = ShipManager([loud(), quiet()])
        both.enabled = True
        only_l = ShipManager([loud()])
        only_l.enabled = True
        only_q = ShipManager([quiet()])
        only_q.enabled = True
        b = both.calculate_aggregate_deterrence_vectorized(px, py, p, base_seed=seed, tick=tick)
        l_res = only_l.calculate_aggregate_deterrence_vectorized(
            px, py, p, base_seed=seed, tick=tick
        )
        q = only_q.calculate_aggregate_deterrence_vectorized(px, py, p, base_seed=seed, tick=tick)
        np.testing.assert_array_equal(b[0], l_res[0])  # quiet fully blocked: both == loud-only
        np.testing.assert_array_equal(b[1], l_res[1])
        if q[0][0] != 0.0:  # quiet would deter alone -> result is not a sum
            assert b[0][0] != pytest.approx(l_res[0][0] + q[0][0])
    ...
    def test_porpoise_count_invariance_subtick(self):
        ...
        px2 = np.array([52.0, 800.0])
        py2 = np.array([50.0, 50.0])  # far porpoise appended
    ...
    def test_moving_ship_weston_subtick_matches_reference(self):
        ...
        from cenop.agents.ship import MAX_DETER_DIST_M, ShipManager, _ship_received_level
        from cenop.landscape.cell_data import create_homogeneous_landscape
        from cenop.parameters.simulation_params import SimulationParameters
    ...
    def test_vectorized_matches_bruteforce_multi(self):
        ...
        from cenop.agents.ship import MAX_DETER_DIST_M, ShipManager
        ...
        mgr = ShipManager(list(ships))
        mgr.enabled = True
        dx, dy = mgr.calculate_aggregate_deterrence_vectorized(px, py, p, base_seed=seed, tick=tick)
        cell = 400.0
        min_m = p.deter_min_distance_ships * 1000.0
        max_m = min(MAX_DETER_DIST_M, p.deter_max_distance * 1000.0)
        n = px.shape[0]
        ...
        for pi in range(n):
            for k in range(1, 31):
                ...
                for s in sorted(ships, key=lambda z: int(z.id)):
                    ...
                    gdx = np.array([px[pi] - sx])
                    gdy = np.array([py[pi] - sy])
                    d = np.array([max(float(np.hypot(gdx[0] * cell, gdy[0] * cell)), 1.0)])
                    if not (min_m < d[0] <= max_m):
                        continue
                    rl = max(
...
class TestSubTickOutOfRangeSlots:
    ...
    def test_long_sweep_excludes_out_of_range_slots_weston(self):
        ...
        for i in range(1, 31):
            ...
            n_in += 1
            rl = _ship_received_level(
                s.noise.get_source_level(), d, np.array([60.0]), np.array([52.0]), p, land, 1, True
            )
            if rl[0] <= p.deter_ships_min_db:
                continue
            vx, vy, _, _, _ = s.deterrence_model.deterrence_components(
...
class TestPausedShipsExcluded:
    ...
    def test_paused_ship_contributes_zero_moving_ship_nonzero(self):
        ...
        _, dy_moving = mgr_moving.calculate_aggregate_deterrence_vectorized(px, py, p, _force_u=0.0)
        _, dy_paused = mgr_paused.calculate_aggregate_deterrence_vectorized(px, py, p, _force_u=0.0)
