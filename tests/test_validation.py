...
class TestPopulationDynamicsValidation:
    ...
    def test_birth_rate_realistic(self):
        ...
        assert (
            mothers_with_calves > 0 or final_pop > initial_pop * 0.9
    ...
    def test_mortality_rate_age_dependent(self):
        ...
        print(
            f"6-month mortality: Juvenile={juvenile_mortality:.1%}, Adult={adult_mortality:.1%}, Elderly={elderly_mortality:.1%}"
        )
        ...
        assert (
            adult_mortality <= max(juvenile_mortality, elderly_mortality) * 1.5
    ...
    def test_energy_distribution_stable(self):
        ...
        landscape = create_homogeneous_landscape(
            width=200, height=200, depth=20.0, food_prob=0.8  # High food availability
        )
        pop = PorpoisePopulation(count=100, params=params, landscape=landscape)
...
class TestDeterrenceValidation:
    ...
    def test_deterrence_probability_distance_decay(self):
        ...
        for i in range(len(received_levels) - 1):
            assert (
                received_levels[i] >= received_levels[i + 1]
    ...
    def test_ship_day_night_difference(self):
        ...
        day_prob = model.calculate_deterrence_probability(
            spl=spl, distance_km=distance_km, is_day=True
        )

        night_prob = model.calculate_deterrence_probability(
            spl=spl, distance_km=distance_km, is_day=False
        )
    ...
    def test_turbine_noise_calculation(self):
        ...
        from cenop.behavior.sound import calculate_received_level, calculate_transmission_loss
    ...
    def test_deterrence_vector_magnitude(self):
        ...
        dx, dy = manager.calculate_aggregate_deterrence_vectorized(px, py, params, cell_size=400.0)
...
class TestSpatialDistributionValidation:
    ...
    def test_porpoises_avoid_land(self):
        ...
        assert (
            pct_in_water >= 0.9
        ), f"At least 90% of porpoises should be in water, got {pct_in_water:.1%}"
    ...
    def test_population_spreads_spatially(self):
        ...
        if np.sum(active) > 0:
            ...
            assert x_std > 1.0 or y_std > 1.0, "Population should spread out spatially over time"
...
class TestPSMValidation:
    ...
    def test_psm_returns_valid_target(self):
        ...
        psm = PersistentSpatialMemory(
            world_width=100, world_height=100, preferred_distance=preferred_dist_km
        )
        ...
        if target is not None:
            ...
            print(
                f"Dispersal target from ({current_x}, {current_y}): ({target_x:.1f}, {target_y:.1f}) at {dist_km:.1f} km"
            )
...
class TestDEPONSComparisonValidation:
    ...
    def test_movement_patterns_realistic(self):
        ...
        params = SimulationParameters(porpoise_count=50)
        landscape = create_homogeneous_landscape(width=300, height=300, depth=20.0, food_prob=0.5)
        pop = PorpoisePopulation(count=50, params=params, landscape=landscape)
...
class TestDeterrenceResponseValidation:
    ...
    def test_displacement_distance_under_deterrence(self):
        ...
        assert final_dist > initial_dist * 0.8, "Porpoises should move away from active turbine"
    ...
    def test_recovery_after_turbine_stops(self):
        ...
        turbine = Turbine(
            ...
            impact=220.0,
            start_tick=0,
            end_tick=50,  # Active for only 50 ticks
        )
        turbine._is_active = True
        turbine.phase = TurbinePhase.CONSTRUCTION
        sim._turbine_manager.turbines.append(turbine)
...
class TestModuleUnitCoverage:
    ...
    def test_dispersal_heading_calculation(self):
        ...
        from cenop.behavior.psm import PersistentSpatialMemory, PSMDispersalType2
        ...
        new_heading = dispersal.calculate_new_heading(current_x=50.0, current_y=50.0)
